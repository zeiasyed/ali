(() => {
  ...
  function playCorrectSound() {
    ...
    // Rising “power blast”
    tone(ctx, { freq: 220, type: "sawtooth", start: 0, dur: 0.1, gain: 0.05, slideTo: 440 });
    tone(ctx, { freq: 440, type: "square", start: 0.08, dur: 0.1, gain: 0.07 });
    tone(ctx, { freq: 554, type: "square", start: 0.16, dur: 0.1, gain: 0.07 });
    tone(ctx, { freq: 659, type: "square", start: 0.24, dur: 0.12, gain: 0.08 });
    // Sparkle shimmer on top
    tone(ctx, { freq: 1320, type: "triangle", start: 0.28, dur: 0.18, gain: 0.045 });
    tone(ctx, { freq: 1760, type: "triangle", start: 0.36, dur: 0.22, gain: 0.03 });
  }

  /** Soft comic “thwip / miss” — playful, not harsh */
  function playWrongSound() {
    const ctx = getAudioCtx();
    if (!ctx) return;
    if (ctx.state === "suspended") ctx.resume().catch(() => {});

    // Quick comic boing down
    tone(ctx, { freq: 320, type: "triangle", start: 0, dur: 0.14, gain: 0.06, slideTo: 140 });
    tone(ctx, { freq: 180, type: "square", start: 0.1, dur: 0.12, gain: 0.04, slideTo: 90 });
  }
